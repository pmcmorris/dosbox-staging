...
struct SDL_Block {
	...
	const char *rendererDriver = nullptr;
	int displayNumber;
	struct {
...
static bool StartUpdate(uint8_t * &pixels, int &pitch)
{
...
{
	const SCREEN_TYPES t = sdl.desktop.type;
	assert(t == SCREEN_SURFACE ? (sdl.surface != nullptr) : true);
	assert(t == SCREEN_TEXTURE ? (sdl.texture.texture != nullptr) : true);
#if C_OPENGL
	assert(t == SCREEN_OPENGL  ? (sdl.opengl.framebuf || sdl.opengl.pixel_buffer_object) : true);
#endif

	const bool success = StartUpdate(pixels, pitch);

	assert(!success || (pixels != nullptr));
	assert(!success || (pitch > 0));
	return success;
}
...
static void DisplaySplash(uint32_t time_ms)
{
	...
	const auto flags = GFX_SetSize(src_w, src_h, GFX_CAN_32, 1.0, 1.0, nullptr);
	if (!(flags & GFX_CAN_32)) {
